# -*- coding: utf-8 -*-
""" Apply SVM to concentric circle data """
...
# Draw original plot
np.random.seed(0)
sin_dat = np.c_[np.sin(2*np.pi*t/N), np.cos(2*np.pi*t/N)]
rand_dat = np.random.randn(N, 2)
X1 = x1d * sin_dat + r_x * rand_dat
X2 = x2d * sin_dat + r_y * rand_dat
X = np.r_[X1, X2]
...
# Draw with SVM edge
clf = SVC(kernel='poly', degree=2, random_state=0, gamma=0.10, C=10.0)
clf.fit(X, y)
resolution = 0.02
...
ax.plot_wireframe(xx1, xx2, ZZ, alpha=0.5, color='gray')
ax.set_zlim(-5,20)
